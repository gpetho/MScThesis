import numpy as np, regex, scipy.stats, pickle
...
class Alignment_old:
    def __init__(self,sl1,sl2,d_model = 'gc'):
        self.segmentations = [sl1,sl2]
        combined_lengths = [sl1.get_combined_lengths(), sl2.get_combined_lengths()]

        margin = int(np.log(len(sl1)) * 2)
        window_l2 = max(0,len(sl2) - len(sl1)) + margin + 1
        window_l1 = max(0,len(sl1) - len(sl2)) + margin + 1

        #self.best_previous = np.empty([len(sl1)+1,len(sl2)+1],dtype=object)
        self.best_previous = shifted_array(len(sl1)+1,window_l1+window_l2,0,window_l1,len(sl1)+1,len(sl2)+1,fill_value=0,dtype=(np.int,2))
        
        #self.D = np.full([len(sl1)+1,len(sl2)+1],np.inf)
        self.D = shifted_array(len(sl1)+1,window_l1+window_l2,0,window_l1,len(sl1)+1,len(sl2)+1)
        self.D[0,0] = 0

        for l1_index in range(0,len(sl1)+1):
            for l2_index in range(max(0,l1_index-window_l1),min(len(sl2)+1,l1_index+window_l2)):
                ...
                for step in STEPS:
                    ...
                    if self.D[si,sj] + STEP_COSTS[step] >= self.D[l1_index,l2_index]:
                        continue
                    if min(step[0],step[1]) == 0:
                        self.D[l1_index,l2_index] = self.D[si,sj] + STEP_COSTS[step]
                        self.best_previous[l1_index,l2_index] = (si,sj)
                    else:
                        d = PartialAlignment.calculate_d(si,sj,step,sl1,sl2,combined_lengths,d_model)
                        if d == np.inf:
                            continue
                        if self.D[si,sj] + d < self.D[l1_index,l2_index]:
                            self.D[l1_index,l2_index] = self.D[si,sj] + d
                            self.best_previous[l1_index,l2_index] = (si,sj)
    ...
    def get_bisegmentation(self):
        return Bisegmentation(self.segmentations,self.bisegments)
...
class Segment:
    def __init__(self,text):
        self.text = text
        self.tokens = np.array([w for w in regex.split('(\W)',text) if len(w) and not w.isspace()])
    ...
    def get_tokens(self, what = 'all', lower = False):
        ...
        if lower:
            return_tokens = np.char.lower(return_tokens)
...
class BisegmentationCorpus:
    ...
    def total_token_count(self):
        return {langs[0]: sum([[len(bisegment.get_tokens(langs[0])) for bisegment in b] for b in self]),
                langs[1]: sum([[len(bisegment.get_tokens(langs[1])) for bisegment in b] for b in self])}
...
anchor_pattern_list_jox_en_hu = [{'minl': i[0],'maxl': i[1], 'l1_pattern': regex.compile(i[2]), 'l2_format': i[3], 'match_indices': i[4]} for i in anchor_segment_patterns_jox_en_hu]